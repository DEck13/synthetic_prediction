...
\textbf{Summary of \cite{clements1996intercept}:} \cite{clements1996intercept} proposes three intercept correction for $m$-ahead forecasts of VECM. They are constant adjustment throughout the $m$ periods, a one-off adjustment, simple adjustment, and tailing-off forecast. In their setup, let $\hat{\boldsymbol{\varepsilon}}_{T}$ be the residual of VECM at $t= T$ and we are interested in  $m$-ahead forecasts. Denote the $m$-ahead forecasts by $\hat{y}_{ T+ m}$ and the adjusted forecasts by $\hat{\boldsymbol{y}}_{T+m}^*$. The \emph{constant adjustment}  considers $\hat{\boldsymbol{y}}_{T+m}^*= \hat{\boldsymbol{y}}_{ T+ m} + \hat{\boldsymbol{\varepsilon}}_{T}$.  The \emph{one-off} adjustment considers $\hat{\boldsymbol{y}}_{T+1}^*= \hat{\boldsymbol{y}}_{ T+ 1} + \hat{\boldsymbol{\varepsilon}}_{T}$ and compute $m$-ahead forecasts using $\hat{\boldsymbol{y}}_{T+1}^*$; i.e., they only adjust once. The \emph{simple adjustment} considers only $\hat{\boldsymbol{y}}_{T+m}^*= \hat{\boldsymbol{y}}_{ T+ m} + \hat{\boldsymbol{\varepsilon}}_{T}$. The \emph{tailing-off} adjustment considers
\begin{align*}
  \hat{\boldsymbol{y}}_{T+m}^*
  &=\hat{\boldsymbol{y}}_{T+m} + \sum_{i=0}^m \mathbf{\Pi}^i \mathbf{H}^{m-1-i}  \hat{\boldsymbol{\varepsilon}}_T,
\end{align*}
where $\mathbf{\Pi}$ is the parameter matrix of  the error correction term. It is shown that the forecast bias is reduced  when the long-run relationship of VECM changes. \\

It is important to note that their method uses $\hat{\boldsymbol{\varepsilon}}_{T}$. In other words, the hope of reducing forecast risk depends on whether a large deviation is observed at $t=T$. That is, the method is not immune to unprecedented and unobserved shock.

\textbf{Summary of \cite{castle2015robust}:} \cite{castle2015robust} proposed a number of robust forecasting in contrast to equilibrium-correction models. However, their proposed methods would depend on whether past data contain future shock signal to adjust for the forecast. As a result, similar to \cite{clements1996intercept}, it is not robust to unprecedented and unobserved shock. \\

\textbf{Summary of \cite{tsay1986time}:} \cite{tsay1986time} specifies a general generating  mechanism of outliers and corresponding outlier detection methods for ARMA model. The two types of his proposed outlier generating mechanism are innovational outlier and additive outlier. The additive outlier only affects observation at $t= T_0$ for some $T_0 \in \{ 1, \ldots, T\}$. In contrast, the innovational outlier introduces a disturbance on the white noise so that it influences the entire process and impacts future values. In our study, in our AR(1) example,  the unprecedented shock is similar to innovational outlier in the sense that the contaminated response will impact future values through autoregressive structure. However, the difference is that the shock is an additive effect. 
...
In our setup, we only model the time series under study up to $t = 1, \ldots, T_1^*+1$.  We do not know what happened  after $T_1^*+1$. Under our setup, there are two possible situations. First, there is no shock after $T_1^*+1$. Second, there are $k$ shocks after $T_1^*$, say, at $T_1^*+d_1, \ldots, T_1^* + d_k$. \\
...
As we talked above, there are two situations. When there is no shock after $T_1^*+1$, we can simply use standard forecast for $m$-ahead  prediction in time series methodologies. When there are $k$ shocks after $T_1^*$, in our method, one should construct  $k$ additional donor pools to estimate the $k$ arriving shock effects. The details are provided in the Discussion Section in the revised manuscript. \\

Suppose that  the existing donor pool has time series that experience $k$ shocks, each of which is similar to the one the time series under study are going to. Then, we can readily apply the above stated method to estimate the \emph{shock transition} and adjust for the forecasts.\\

\begin{tcolorbox}
  It would be interesting empirically to see if the shock is anticipated to be permanent or transitory.
\end{tcolorbox}

In fact, it is more of a modeling assumption. If we assume an AR model, the impact of any shock will diminish as time passes by. However, if we assume an additive model, the impact will be permanent.
...
To our best knowledge, our theoretical results are not related to when the transitory shocks die out. In our proposition, the risk reduction does not depend on the autoregressive parameter $\phi_1$, which controls the rate how the impact of the shock dies out. Our proposition is related to the adjustment at time point $T_1^*+1$ but not in a dynamic fashion.
} \\
