...
\begin{figure*}[t!]
  \begin{center}
    \subfloat[]{\includegraphics[height = 5.5cm]{FIG1a.pdf}}\\
     \subfloat[]{\includegraphics[height = 5.5cm]{FIG1b.pdf}}
  \end{center}
...
%Note that the two forecasts do not differ in their predictions for $y_{1,t}$, $t = 1,\ldots T_1^*$. Instead, they only differ in predicting $y_{1,T_1^*+1}$. 
...
Unbiasedness properties for $\E{\alpha_1}$ of $\hat{\alpha}_{\rm adj}$ and $\hat{\alpha}_{\rm wadj}$ allow for simple conditions for risk-reduction to hold, and more importantly motivates a bootstrap estimation procedure for evaluation of these conditions, discussed in Section \ref{conditions} and \ref{varbootstrap} respectively. Next, we present the variance expressions for $\hat{\alpha}_{\rm adj}$ and $\hat{\alpha}_{\rm wadj}$ as below: 
...
Apple Inc. is an American technology company that develops a range of electronics and software with typical products such as, iPhone, iPad, Mac, and Apple Watch. Every year, Apple Inc. releases a new series of MacBook computers to the public. Typically, after the release, the stock price of Apple Inc. is expected to experience a shock since Macbook sales is one of the most profitable line of Apple Inc.'s business and investors are likely to adjust their investment decisions based on the new features introduced. For example, the sales of Macbooks account for 9.8\% of its revenue \citep{applemac}. In this example, we are interested in forecasting the stock price of Apple Inc. after the release of the latest MacBook on November, 10th, 2020. In recent years, we observe that it takes two or more days (\red{Why is it two or more days and not just two days? \textcolor{blue}{It took two trading days for shock effects for 2017 and 2018 but 6 trading days for 2019.} How can we be certain that we want the shock effect on November 11th instead of November 12th or some later date?} \textcolor{blue}{We cannot know how long it took for the shock effect in 2020 to take an effect. From the past data (i.e., data in 2017 and 2018), I think 2 trading days are a safe guess.}) for the shock to take effect on the stock price. Therefore, we are interested in the stock price of Apple Inc. on the day after the release date, i.e., on November 11th, 2020. The detailed considerations of our analysis are outlined as below.

\begin{enumerate}[label = (\arabic*)]
  \item \textbf{Selection of model.}  We use an AR(1) model to forecast the stock price of Apple Inc. We consider 20 trading days data for the time series of interest and time series in the donor pool. All the estimates will be adjusted for inflation. Since we are interested in the stock price of the day after the release date, consider $D_{i, t} = I(t \in \{T_i^* +1, T_i^*+2\})$. As a result, we consider weighting with respect to  
  \begin{align*}
    ...
    \end{pmatrix},
  \end{align*}
  where $ \mathbf{x}_{i,T_i^*+1: T_i^*+2}\in \reals^{p \times 2}$ denotes the matrix joining both covariate column vectors corresponding to time points $t = T_i^* +1$ and $t = T_i^*+2$. \red{You need to define the space of possible values for possible weighs in this new context, it is not immediately clear what you did.} \textcolor{blue}{In this case, $\mathcal{W}= \{\mathbf{W}\in [0,1]^n\colon 1_n'\mathbf{W}=1\}$ and we aim to find $\mathbf{W}^*\in \mathcal{W}$ with $\mathbf{W}^*=(w_2^*,\ldots, w_{n+1}^*)'$ such that }
  \begin{align*}
    \textcolor{blue}{\mathbf{x}_{i, T_i^*+1:T_i^*+2}=\sum_{i=2}^{n+1}w_i^*\mathbf{x}_{i, T_i^*+1:T_i^*+2}}.
  \end{align*} 
  \textcolor{blue}{and $\mathbf{W}^*$ is estimated by}
  \begin{align*}
 \textcolor{blue}{\mathbf{W}^*= \argmin_{\mathbf{W}\in \mathcal{W}} \left\| \mathbf{x}_{i, T_i^*+1:T_i^*+2}-\sum_{i=2}^{n+1}w_i^*\mathbf{x}_{i, T_i^*+1:T_i^*+2} \right\|_{2p}.}
  \end{align*}
The model $\mc{M}_2$ is then changed to 
  \begin{align*}
    \mc{M}_2 \colon \begin{array}{l}
  y_{i,t} =\eta_i +\alpha_i  I(t \in \{T_i^* +1, T_i^*+2\}) + \phi_i y_{i, t-1} + \theta_i'\mbf{x}_{i,t} + \varepsilon_{i,t}\\[.2cm]
  \; \textcolor{red}{\alpha_i 1_2= \mu_{\alpha}1_2+\mbf{x}_{i, T_i^*+1:T_i^*+2}'\delta_{i}+ \t{\varepsilon}_{i}1_2},
\end{array}
  \end{align*}
  where \textcolor{red}{$\delta_i \in \R^{p \times 1}$ is not assumed to be random}. \red{The above does not make sense, $\delta_{i}'\mbf{x}_{i, T_i^*+1:T_i^*+2}$ is not a scalar, and $y_{i,t}$ is not a vector, right?  It seems that, as currently written, $\alpha_i$ should be replaced by $\alpha_{i,t}$, for $t = T^*+1, T^*+2$, where it is understood that $\alpha_{i,T^*+1}$ is close to zero. But I am not even sure if that is correct, why is the shock effect for the irrelevant nuisance next day response even considered at all? What did you actually do here? \textcolor{blue}{I change the model accordingly where we cannot assume $\delta_i$ is random anymore in this context and it has to be particular $\delta_i$ such that the model holds. It is also possible that  we can use $\alpha_{i,t}$. By taking it two days to take effect, I mean $\alpha_{i, T_i^*+1}$ is small but not close to zero and it should be taken into consideration together with $\alpha_{i, T_i^*+2}$.  I personally prefer $\alpha_i$ instead of $\alpha_{i,t}$ since $\alpha_i$ is more intuitive  in that there is only one gross shock effect. Or, there is another possible approach. Recall that we can use different covariates in the model of $\alpha_i$  and in the model of $y_{i,t}$. So, we can only use $\mathbf{x}_{i, T_i^*+1}$ to construct the weights. Please let me know what you think and which model you prefer.}}
  ...
  \item \textbf{Construction of donor pool.} Our donor pool consists of the time series data of Apple Inc. with shock-effect date in the middle of 15 trading days for 2017, 2018, and 2019.  The specific shock-effect dates are June 6th, 2017, October 29th, 2018, and June 3rd, 2019, respectively. We only include the last three Macbook releases in our donor pool because those releases are similar to the current release in the sense that Apple inc. has only updated the hardware of the computer without much redesign in these releases \citep{apple2017, apple2018, apple2019, apple2020}. %The new MacBook series is built with M1 chip invented by Apple inc. instead of chips by Intel inc. that were used in past years, and their CPU speed improves significantly \citep{apple2020}. 
\end{enumerate}

The covariates and response of time series in the donor pool are adjusted for inflation. The estimated shock effects $\alpha_i$ are $0.864, 0.905, 1.129$ for $i = 2, 3, 4$, respectively. We centered and scaled the covariates in weights computation but not in the model of $y_{i,t}$. As a result, we observe $\mathbf{W}^* = (0.664, 0.000, 0.336)$ \red{It is still a little confusing where this came from since it is not defined in this setting.} and $
  \|\mbf{X}_1-\hat{\mbf{X}}_1(\mbf{W}^*)\|_2 = 3.674$. It appears that  the shock effect of interest is similar to 2017 and 2019 shock effects. The resulting shock effect estimates are $\hat{\alpha}_{\rm adj}= 1.071$, $\hat{\alpha}_{\rm wadj}=1.066$, and $\hat{\alpha}_{\rm IVW} = 1.090$. The bootstrap estimates variances for variances for $\hat{\alpha}_{\rm adj}$, $\hat{\alpha}_{\rm wadj}$,  and $\hat{\alpha}_{\rm IVW}$ are $0.192$, $0.095$, and $0.091$, respectively. Our risk-reduction propositions yield that those shock-effect estimators reduce the post-shock forecast risk. As we can see from Figure \ref{Fig3}, the post-shock forecasts perform well. The unadjusted forecast misses the post-shock by 1.518 dollars, while the use of $\hat{\alpha}_{\rm adj}$, $\hat{\alpha}_{\rm wadj}$, and $\hat{\alpha}_{\rm IVW}$ only miss by 0.447, 0.427, and 0.452 dollars, respectively. Note  that the 1-ahead forecast error for the covariates contribute to some of the forecast risk.
