...
In this section, we will describe the assumed autoregressive models with random effects for which 
...
\end{array}
  ...
  \item \textbf{Selection of covariates.} We consider lagged  S\&P 500 index prices and 13 Week treasury bill rates as covariates. In this example, the weighting method will require future covariates in computation. As a result, forecasting of future covariates is needed. We used AIC to select AR(3) for S\&P 500 index prices and AR(1)  for 13 Week treasury bill rates. We used Ljung-Box test to test  model adequacy for 10 lags and failed to reject the null hypothesis that autocorrelation does not exist in the residuals series of the corresponding models. Then, we use 1-ahead forecasts to predict the future covariates.
  \item \textbf{Construction of donor pool.} Our donor pool consists of the time series data of Apple Inc. with shock-effect date as  in the middle of 15 trading days for 2017, 2018, and 2019.  The shock-effect dates are June 6th, 2017, October 29th, 2018, and June 3rd, 2019, respectively. Those dates are the past dates when Apple Inc. introduced new series of MacBook computers in the corresponding year.
\end{enumerate}

The covariates and response of time series in the donor pool are adjusted for inflation. The estimated shock effects $\alpha_i$ are $0.864, 0.905, 1.129$ for $i = 2, 3, 4$, respectively. We centered and scaled the covariates in weights computation but not in the model of $y_{i,t}$. As a result, we observe $\mathbf{W}^* = (0.664, 0.000, 0.336)$ and $
  \|\mbf{X}_1-\hat{\mbf{X}}_1(\mbf{W}^*)\|_2 = 3.674.
$ It appears that  the shock effect of interest is similar to 2017 and 2019 shock effects. 

The resulting shock effect estimates are $\hat{\alpha}_{\rm adj}= 1.071$, $\hat{\alpha}_{\rm wadj}=1.066$, and $\hat{\alpha}_{\rm IVW} = 1.090$. The bootstrap estimates variances for variances for $\hat{\alpha}_{\rm adj}$, $\hat{\alpha}_{\rm wadj}$,  and $\hat{\alpha}_{\rm IVW}$ are $0.192$, $0.095$, and $0.091$, respectively. Our risk-reduction propositions yield that those shock-effect estimators reduce the post-shock forecast risk.
