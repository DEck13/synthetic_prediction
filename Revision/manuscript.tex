...
Apple Inc. is an American technology company that develops a range of electronics and software with typical products such as, iPhone, iPad, Mac, and Apple Watch. Every year, Apple Inc. releases a new series of MacBook computers to the public. Typically, after the release, the stock price of Apple Inc. is expected to experience a shock since Macbook sales is one of the most profitable line of Apple Inc.'s business and investors are likely to adjust their investment decisions based on the new features introduced. For example, the sales of Macbooks account for 9.8\% of its revenue \citep{applemac}. In this example, we are interested in forecasting the stock price of Apple Inc. after the release of the latest MacBook on November, 10th, 2020. \textcolor{red}{From knowledge from our donor pool (see  (3) Construction of donor pool below), we observe that it takes two days for the shock effect to take hold.} Therefore, we are interested in the stock price of Apple Inc. on the day after the release date, i.e., on November 11th, 2020, \red{when we conjecture that the time series of interest would experience a shock}. \red{We collect our data through   \texttt{quantmod} package \citep{ryan2017quantmod} in R.} The detailed considerations of our analysis are outlined as below.

\begin{enumerate}[label = (\arabic*)]
  \item \red{\textbf{Selection of model.}}  We use an AR(1) model to forecast the stock price of Apple Inc. All the estimates will be adjusted for inflation.  We consider $D_{i, t} = I(t = T_i^*+2)$. By observing  the data, we found a large shock effect on October 10th, 2020, two days before the release of iPhone 12, iPhone 12 Pro, MagSafe accessories, and HomePod mini. Denote this date by $T_{1,\#}$, and the shock effect by $\beta$. We account for this shock effect that is not of our interest in the model by setting 
  \begin{align*}
    y_{1,t} &= \eta_i + \alpha_1 D_{1,t} +\phi_1y_{1, t-1}+\theta_1'\mathbf{x}_{1,t} + \beta I(t = T_{1,\#}) + \varepsilon_{i,t}.
  \end{align*}
  The model for $y_{i,t}$ is the same as in Section \ref{modelsetup} for $i = 2, \ldots, n+1$.   The model for the shock effect is
  \begin{align*}
  ...
  \item \textbf{Construction of donor pool.} Our donor pool consists of the time series data of Apple Inc. with shock-effect date in the middle of \red{25} trading days for 2017, 2018, and 2019.  The specific shock-effect dates are June 6th, 2017, October 30th, 2018, and June 3rd, 2019, respectively. We only include the last three Macbook releases in our donor pool because \red{we conjecture that} those releases are similar to the current release in the sense that Apple inc. has only updated the hardware of the computer without much redesign in these releases \citep{apple2017, apple2018, apple2019}; \red{and it is indeed consistent \citep{apple2020}}.  %The new MacBook series is built with M1 chip invented by Apple inc. instead of chips by Intel inc. that were used in past years, and their CPU speed improves significantly \citep{apple2020}. 
\end{enumerate}
