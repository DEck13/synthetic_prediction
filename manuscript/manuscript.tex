...
\begin{table}[H]
  ...
  \begin{center}
    ...
    Leave-one-out consistency & 0.000 & 0.800 & 0.000
    \end{tabular}
  \end{center}  
\end{table}
\vspace{-.5cm}

Using the parametric bootstrap conditioned on the donor pool proposed in Section \ref{varbootstrap}, we estimated parameters for risk-reduction propositions and risk-reduction quantities proposed in  Section \ref{properties}; and the result is presented in Table \ref{table2}. Plugging these estimates into conditions in Section \ref{properties} and risk-reduction formulas in Section \ref{comparisons} yields (1) $\hat{\alpha}_{\text{adj}}$, $\hat{\alpha}_{\text{wadj}}$, and $\hat{\alpha}_{\text{IVW}}$  reduce the risk and (2) the risk-reduction quantities are $34.565$, $34.880$, and $34.139$, respectively. The estimated risk-reduction quantities  vote $\hat{\alpha}_{\rm wadj}$  for the best estimator. Moreover, the leave-one-out consistency of the risk-reduction propositions are 0.000, 0.800, and 0.000 for $\hat{\alpha}_{\text{adj}}$, $\hat{\alpha}_{\text{wadj}}$, and $\hat{\alpha}_{\text{IVW}}$, respectively. It implies that we should only use the propositions for $\hat{\alpha}_{\text{wadj}}$. Then, we verify the consistency of the result yielded by risk-reduction propositions with the reality as below.
...
Therefore, the risk-reduction propositions are consistent with the reality  with the reduced risks for forecasts using  $\hat{\alpha}_{\text{adj}}$, $\hat{\alpha}_{\text{wadj}}$, and $\hat{\alpha}_{\text{IVW}}$ than without. Moreover, the risk-reduction quantities are consistent with the reality as well. The results are also consistent with leave-one-out consistency for $\hat{\alpha}_{\text{wadj}}$ in that it  reduces the risk. 
