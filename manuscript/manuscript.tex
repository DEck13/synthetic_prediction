...
The parametric bootstrap without conditioning on the donor pool differs from the one with in the step that the former performs a non-parametric bootstrap by sampling $n$ values from $I=\{2, \ldots, n+1\}$ without replacement to generate  $I^*$. Then, it follows the same step with the one conditioned on the donor pool by replacing $I$ by $I^*$. 
...
\subsection{Parametric bootstrap simulation for risk-reduction propositions}
...
\begin{center}
  \begin{center}
      \begin{tabular}{cc|c|c}
        ...
        & & Accept $\hat{\alpha}$ & Reject $\hat{\alpha}$ \\ 
                \hline
