...
One can then use the bootstrapped shock-effects estimated by our residual bootstrap to provide an approximation for parameters involved in the risk-reduction conditions in Propositions \ref{proprisk} and \ref{propriskwadj2}. 

$\mc{B}_u$ is the same as above with the addition of first round of sampling $n$ values from $I=\{2, \ldots, n+1\}$ without replacement to generate  $I^{(b)}$. Then, it follows the same step with the one conditioned on the donor pool by replacing $I$ by $I^{(b)}$. Care is needed when using this bootstrap procedure to estimate the variability of $\hat{\alpha}_{\mrm{wadj}}$. We would assume $\mathbf{W}^*$ is non-degenerate (i.e., no zero weights) in the population when using $\mc{B}_u$.  See Section \ref{discussion} for discussions.
...
Now, we discuss some consistency results for $\mc{B}_c$ conditioned on $\Theta_1$. Let 
$
...
\end{align*}
The case for the sample variance of the bootstrapped weighted adjustment estimator is similar. The same rationale holds for the adjustment estimator as well since it is a linear combination of OLS estimates, and the weights are not random conditioned on $\Theta$. However, the case for $\hat{\alpha}_{\rm IVW}$ is slightly different because its weights are random. It is not clear with respect to whether similar consistency results holds for $\hat{\alpha}_{\rm IVW}$. We would only claim that our bootstrap procedure provides an approximation for the case of $\hat{\alpha}_{\rm IVW}$.
...
In particular, $\overline{\hat{\alpha}_{\rm wadj}}$ will provide an approximation for $\E{\alpha_1}$ under $\mc{M}_1$, $\mc{M}_{21}$, and $\mc{M}_{22}$ since it is unbiased for $\E{\alpha_1}$ under those three configurations from Proposition \ref{unbiased}. Therefore, we can estimate the risk-reduction quantities in Section \ref{comparisons} using the estimates from the parametric bootstrap. For example, the $\Delta_{\rm adj}$ and its estimator $\hat{\Delta}_{\rm adj}$ are
\begin{align*}
  \Delta_{\rm adj} 
  &= (\E{\alpha_1})^2 -\var{\hat{\alpha}_{\rm adj}} -(\E{\hat{\alpha}_{\rm adj}}-\E{\alpha_1})^2 \\
  \hat{\Delta}_{\rm adj} 
 & = (\overline{\hat{\alpha}_{\rm wadj}})^2 -S^2_{\hat{\alpha}_{\rm adj}} -(\overline{\hat{\alpha}_{\rm adj}}-\overline{\hat{\alpha}_{\rm wadj}})^2,
\end{align*}
where $S^2_{\hat{\alpha}_{\rm adj}}$ is the bootstrap sample variance for $\hat{\alpha}_{\rm adj}$. Suppose the $\hat{\Delta}_{\rm wadj}$ and $\hat{\Delta}_{\rm IVW}$ are estimators for $\Delta_{\rm wadj}$ and $\Delta_{\rm IVW}$, respectively. Then, we can evaluate the risk-reduction propositions by judging whether $\hat{\Delta}_{\rm est}>0$ for $\text{est} \in \{\text{adj}, \text{wadj}, \text{IVW}\}$, and  the best estimator $\hat{\alpha}_{\rm best}$ is the one with the maximum estimated risk-reduction quantities.
...
In this section, we provide justification for our methods based on Monte Carlo simulation examples. We implemented our simulation based on $\mc{M}_{22}$ with negligibly small $\Sigma_{\gamma}$ and $\Sigma_{\delta}$ approximating the design of $\mathcal{M}_{21}$.  We consider $p=13$ and $\mu_{\alpha}=10$, where $p = 13$ is set to satisfy conditions in Proposition \ref{uniqueness}.

Parameter setup of our simulations are detailed as below. $\phi_i$'s are sampled independently from $\mrm{Uniform}(0,1)$. We  sampled $T_i$'s  independently from  $\Gamma(15, 10)$ that are further rounded to integers. If it exists some  $i$ such that  $T_i < 90$, we let $T_i=90$ instead.  Moreover, $T_i^*$ is randomly selected from $2p + 4$ to $T_i-1$. Those are set up to satisfy a necessary condition for the design matrix of OLS estimation to have full rank. Moreover, it is also designed to illustrate the performance of $\hat{\alpha}_{\rm IVW}$ that may perform well in time series with varying lengths. However, it is still possible that the design matrix for the bootstrapped model does not have full rank for some $b\in \{1, \ldots, B\}$. In that case, we do not consider such $b$ in computations of means and variances. Additionally, we generated the covariates from $\Gamma(1,10)$ to set up a setting when the $\hat{\alpha}_{\rm wadj}$ may perform well.  


In this experiment, we consider parameter setup by varying $\sigma$ in the model of $y_{i,t}$, $n$, the donor pool size, and $\sigma_{\alpha}$ in the model of $\alpha_i$. We used 200 replications of Monte Carlo simulations and $B = 500$ for computation; and report means and standard errors for estimated quantities accordingly.


\subsection{Bootstrap simulation for risk-reduction propositions}
\label{parametricbootstrapsimulation}

In this section, we provide justification for the bootstrap procedures used in estimating parameters for risk-reduction propositions (see Section \ref{properties}) and risk-reduction quantities (see Section \ref{comparisons}). We will compare the result of using parametric bootstrap conditioned on the donor pool with the one without using the parameter setup discussed in Section \ref{simulation}.

Specifically, we verify the unbiasedness results in Proposition \ref{unbiased}, and consistency of our proposed risk-reduction propositions. We define the consistency by whether our risk-reduction propositions yield results that agree with the reality that can be known in the simulation. Suppose we make the decision to  accept $\hat{\alpha}$ for $\hat{\alpha}\in \{\hat{\alpha}_{\rm adj}, \hat{\alpha}_{\rm wadj}, \hat{\alpha}_{\rm IVW}\}$ if risk-reduction proposition reports that $\hat{\alpha}$ reduces the risk, and reject $\hat{\alpha}$ otherwise. Consistency can be interpreted in a similar way to hypothesis testing with the following confusion table. 
\begin{center}
  \begin{center}
      \begin{tabular}{cc|c|c}
        ...
        & & Use $\hat{\alpha}$ & Do not use $\hat{\alpha}$ \\ 
                \hline
     \multirow{2}{*}{Truth}  & $R_{T_1^*+1,2}<R_{T_1^*+1,1}$ & Consistent & Inconsistent\\
      \cline{3-4}
      & $R_{T_1^*+1,2}>R_{T_1^*+1,1}$   & Inconsistent & Consistent \\
      \hline
      \end{tabular}
  \end{center}
\end{center}
We report 1 for risk-reduction propositions to be consistent and 0 otherwise. Recall that in Section \ref{comparisons}, we could estimate the risk-reduction quantities, and therefore select the best estimator with the maximum risk-reduction quantities. It is also possible to confirm whether this decision is true in a simulation setting. Similarly, we report 1 for the decision of selecting the best shock-effect estimators  to be consistent and 0 otherwise.

In the first experiment, we consider the parameter combination of  $n \in \{5, 10, 15, 25\}$ and $\sigma_{\alpha} \in  \{1, 5, 10, 25, 100\}$ where we fix $\sigma$ to 1. The bias results are not reported for $\hat{\alpha}_{\rm IVW}$ since its moments are generally unknown. It is shown in Table ? that as the donor pool size increases, the bias and consistency results improve with more precision. In contrast, the result gets poorer as  the variance of $\alpha_i$ increases. It is to be expected since the consistency results claimed in Section \ref{varbootstrap} are conditioned on $\Theta$. Under a fixed design, it is evident that the results of our risk-reduction propositions are highly consistent if the models for $\alpha_i$'s  are identified correctly with appropriate covariates. But it is reversed in the random design case. Moreover, notice that the consistency for voting the best estimator works poor when $n$ is small but improves significantly when $n$ is large.

\subsection{Simulation for prediction}

In this section, we report prediction results for our methods in the simulation. Specifically, we are interested in the distance between $\alpha_1$ and our shock-effect estimators, and the distance between $y_{1,T_1^*+1}$ and $\hat{y}_{1,T_1^*+1}$, $\hat{y}_{1,T_1^*+1}+ \hat{\alpha}_{\rm adj}$, $\hat{y}_{1,T_1^*+1}+\hat{\alpha}_{\rm wadj}$, and $\hat{y}_{1,T_1^*+1}+\hat{\alpha}_{\rm IVW}$.

In the first experiment, Table ? shows similar results to  Section  \ref{parametricbootstrapsimulation} that the prediction and distance to $\alpha_1$ gets better as $n$ increases but worser when the variance of $\alpha_i$ gets larger.
...
\end{align*}
Note that the norm is computed using the $k$-dimensional Euclidean metric. The fit is acceptable. According to discussions in Section \ref{constructionofestimators}, the property of $\hat{\alpha}_{\rm wadj}$ should hold approximately. The solution $\mathbf{W}^*$ reports that the $\alpha_1$ is similar to $\alpha_3$ (September 8, 2008) and $\alpha_5$ (November 27, 2014) under the setup of $\mc{M}_2$ and our collected covariates. 
...
Using the parametric bootstrap proposed in Section \ref{varbootstrap}, we estimated parameters for risk-reduction propositions and risk-reduction quantities proposed in  Section \ref{properties}; and the result is presented in Table \ref{table2}. Plugging these estimates into conditions in Section \ref{properties} and risk-reduction formulas in Section \ref{comparisons} yields (1) $\hat{\alpha}_{\text{adj}}$, $\hat{\alpha}_{\text{wadj}}$, and $\hat{\alpha}_{\text{IVW}}$  reduce the risk and (2) the risk-reduction quantities are $28.031$, $28.084$, and $25.748$, respectively. The estimated risk-reduction quantities  vote $\hat{\alpha}_{\rm wadj}$  for the best estimator. Note that this parametric bootstrap procedure is conditioned  on the  donor pool. Then, we verify the consistency of the result yielded by risk-reduction propositions with the reality as below.
...
the use of $\hat{\alpha}_{\text{adj}}$, $\hat{\alpha}_{\text{wadj}}$, and $\hat{\alpha}_{\text{IVW}}$ have  RMSE of $4.436$, $4.441$, and $4.423$ dollars, respectively.
Therefore, the risk-reduction propositions are consistent with the reality  with the reduced risks for forecasts using  $\hat{\alpha}_{\text{adj}}$, $\hat{\alpha}_{\text{wadj}}$, and $\hat{\alpha}_{\text{IVW}}$ than without. However, the risk-reduction quantities are inconsistent with the reality. The empirical MSE votes for $\hat{\alpha}_{\text{IVW}}$ though the estimates of  $\hat{\alpha}_{\text{adj}}$, $\hat{\alpha}_{\text{wadj}}$, and $\hat{\alpha}_{\text{IVW}}$ are fairly close shown by Figure~\ref{Fig:CP}.  It is consistent with the finding in Section \ref{parametricbootstrapsimulation} that voting for best estimator with estimated risk-reduction quantities is very likely to be inconsistent with the truth when the donor pool size $n$ is small.
...
Similarly, the mean function for $\alpha_i$ under $\mc{M}_2$ can be extended to include more lagged predictors. Moreover, the functional form for the mean function can be extended beyond the linear regression model under $\mc{M}_2$. Multiple shock-effects can be nested within a time series; and time series in the donor pool are allowed be dependent. As an example, we could consider a dependency structure for the September 2008 shock effects in our analysis of Conoco Phillips stock.
