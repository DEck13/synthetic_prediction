...
# lagged covariates
WTI_Close_lag1 <- COP_close$WTI_Close[-nrow(COP_close)]
GSPC_Close_lag1 <- COP_close$GSPC_Close[-nrow(COP_close)]
# re-input
COP_close <- data.frame(COP_close[-1, ], WTI_Close_lag1, GSPC_Close_lag1)
...
# time window
TS3 <- COP_close[which(COP_close$Date == "2008-07-28"):which(COP_close$Date == "2008-09-26"), ]
...
Tstar <- which(COP_close$Date %in% Tstar.Date)
# X1
...
summ <- function(x) {
  ...
  # Std. dev
  Std <- sd(x, na.rm = TRUE)
  # 0, 25, 50, 75, 100 th quantile
  quarters <- quantile(x, probs = c(0, 0.25, 0.5, 0.75, 1))
  # result
  result <- c(N, Mean, Std, quarters)
  # rename
  names(result) <- c("N", "Mean", "Std. dev.", "Min.", 
                     "25th Perc.", "Median", "75th Perc.", "Max.")
  # round to the 2nd digits
...
summary <- c()
for (i in 1:4) {
  covariates <- c('GSPC_Close', 'WTI_Close', 'Y')
  summaryi <- c()
  for (j in 1:3) {
    summaryi <- cbind(summaryi, t(summ(TS[[i]][,covariates[j]])[2:3]))
  }
...
m_COP_03_06_20 <- lm(Y ~ COP_Close + GSPC_Close + WTI_Close + GSPC_Close + WTI_Close + 
                     WTI_Close_lag1 + GSPC_Close_lag1, data = TS1[-nrow(TS1), ])
# Yhat 1
Yhat_nothing <- coef(m_COP_03_06_20) %*% t(as.matrix(cbind(1, as.vector(TS1[nrow(TS1), c(2, 3, 4, 6, 7)]))))
# Yhat 2
Yhat_adj <- c(alpha_adj + Yhat_nothing, alpha_IVW + Yhat_nothing, alpha_wadj + Yhat_nothing)
Yhat_additive <- c(alpha.adj.additive + Yhat_nothing, alpha.IVW.additive + Yhat_nothing, 
