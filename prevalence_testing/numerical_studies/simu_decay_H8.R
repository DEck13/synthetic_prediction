...
system.time(suppressMessages(
	output_mu_H8 <- lapply(1:nrow(sim_params), FUN = function(j) {
		# parameters
...
# simulation time
system.time(suppressMessages(
	output_ell_H8 <- lapply(1:nrow(sim_params), FUN = function(j) {
		# parameters
		ell <- sim_params[j, 1]
		H <- sim_params[j, 2]
		# %do% evaluates sequentially
...
save(output_mu_H8, output_ell_H8, file = "output_decay_H8.RData")
