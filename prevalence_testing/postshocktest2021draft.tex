...
\end{split}
\end{equation}
%where the distributions $\mathcal{F}_{\tilde{\mathbf{\phi}}_j}(\x_{i,T_i^*+1})$ and $\mathcal{F}_{\tilde{\mathbf{\theta}}_{i'}}(\x_{i,T_i^*+1})$ are such that $\mrm{E}(\mathcal{F}_{\tilde{\mathbf{\phi}}_i}(\x_{i,T_i^*+1})) - \mrm{E}(\mathcal{F}_{\tilde{\mathbf{\phi}}_j}(\x_{i',T_i^*+1})) \to 0$ as $\|\x_{i,T_i^*+1} - \x_{i',T_i^*+1}\| \to 0$.
...
In this paper, we propose a new way of aggregating information from donor pool to conduct post-shock persistence testing, which is quite different from \cite{lin2021minimizing}. The general structure of this aggregation technique is illustrated as below. Suppose we are given a nonempty donor pool with size $n$ that consist of donors $\mc{T}_2, \ldots, \mc{T}_{n+1}$, whose data, including shock information, are observed. With shock information, we are able to compute unadjusted forecast that does not incorporate shock information and adjusted forecast that does. The forecast comparison using methodology of \cite{quaedvlieg2021multi} for each donor yields a $p$-value, $p_{i+1}$ for $i$th donor in the donor pool. Given $\{p_i \colon i = 2, \ldots, n+1\}$, we propose a methodology for judging whether the shock of the time series of interest is persistent or not. Next, we illustrate the details of our methodology as below.



For each donor $\mc{T}_i$, $i = 2, \ldots, n+1$, to compare adjusted and unadjusted forecasts,  a model needs to be specified. The model should describe how the shock impacts the time series. Note that the model needs not to be the same for every donor in the donor pool. However, certain conditions need to be imposed as discussed previously.  Given a model for each donor pool, we compute the unadjusted forecast and adjusted forecast. For the sake of generality, unadjusted forecast and adjusted forecast are two different principles of forecast with one ignoring the shock and the other accommodating it.  From our best knowledge, in the literature of post-shock prediction, the reliable post-shock point forecast exists only for $t=T^*+1$ when the shock occurs. The adjusted forecast and unadjusted forecast are the same for $t\leq  T^*$ whereas the adjusted forecast and unadjusted forecast differs by an adjustment that depends on the model chosen. 
...
\begin{prop}
	...
	$$
	  \left\vert\E(y_{i,t} - y_{i',t})\right\vert \leq \epsilon,
	$$
	provided that 
	\begin{align*}
