...
\mc{M} \colon \begin{array}{l}
  y_{i,t} =\eta_i + \sum_{j=1}^{q_1}\phi_{i,j}y_{i, t-j} + \sum_{j=0}^{q_2-1}\theta_{i,j+1}'\mbf{x}_{i,t-j} + f(\mc{F}_{i,t-1},\x_{i,t-1},\alpha_i)D_{i,t} + \varepsilon_{i,t},\\[.2cm]
  \; f(\mc{F}_{i,t-1},\x_{i,t-1},\alpha_i) = \alpha_i g(t) + \sum_{j=1}^{q_3}\t\phi_{i,j}y_{i, t-j} + \sum_{j=0}^{q_4-1}\t\theta_{i,j+1}'\mbf{x}_{i,t-j}, \\[.2cm]
  \; \alpha_i = \mu_{\alpha}+ \sum_{j=1}^{q_5}\delta_{i,j}'\mbf{x}_{i, T_i^*+j}+ \t{\varepsilon}_{i}, 
\end{array}\label{model}
\end{align}
where $D_{i,t} = I(t \geq T_i^* + 1)$, $I(\cdot)$ is the indicator function, $\x_{i,t} \in \R^{p}$ are fixed with $p \geq 1$, $g(t)$ is a bounded continuous function which does not cross zero and $\lim_{t\to\infty}g(t) = a \in \R$. Let 
...
$\mathbf{\delta}_i = (\delta_{i,1},\ldots,\delta_{i,q_5})'$, 
...
\tilde{\mathbf{\phi}}_i \indep \tilde{\mathbf{\theta}}_i \indep \mathbf{\delta}_i \indep \varepsilon_{i,t} \indep \tilde\varepsilon_{i,t}.
\end{align*}
...
\begin{align}
\begin{array}{l}
 \;\, \Theta = \{(\eta_i, \mathbf{\phi}_i, \mathbf{\theta}_i, \tilde{\mathbf{\phi}}_i, \tilde{\mathbf{\theta}}_i, \mathbf{\delta}_i, \alpha_i, \mbf{x}_{i,t}, y_{i,t-1})\colon    t= 1, \ldots, T_i, i = 2, \ldots, n +1\} \\
   \Theta_1 = \{(\eta_i, \mathbf{\phi}_i, \mathbf{\theta}_i, \tilde{\mathbf{\phi}}_i, \tilde{\mathbf{\theta}}_i, \mathbf{\delta}_i, \alpha_i, \mbf{x}_{i,t}, y_{i,t-1})\colon  t= 1, \ldots, T_i, i = 1\}
