...
We now bound the difference in loss differential vectors $\|\dbf_{1,t} - \dbf_{i',t}\|$ for $t > T^*$ when we specify the loss $L(a,b) = |a-b|$. The loss differential vector $\dbf_{1,t} \in \R^H$ has elements $|y_{1,t} - \hat{y}^{1,h}_{1,t}| - |y_{1,t} - \hat{y}^{2,h}_{1,t}|$. What we have showed so far in \eqref{bounds} and \eqref{forecastbounds} allows us to write
$$
  |y_{1,t} - \hat{y}^{1,h}_{1,t}| - |y_{1,t} - \hat{y}^{2,h}_{1,t}| \leq |\tilde{y}_{i',t} - \hat{\tilde{y}}^{i',h}_{1,t}| - |\tilde{y}_{i',t} - \hat{\tilde{y}}^{2,h}_{1,t}| + 2(\varepsilon_3 + \varepsilon_7).
$$
Let $\varepsilon_8 = 2(\varepsilon_3 + \varepsilon_7)$ and conclude that $\|\dbf_{1,t} - \dbf_{i',t}\| \leq \varepsilon_8$ on $E$ for all $t > T^*$. Putting this all together, we can pick $\varepsilon_8$ small enough so that for some fixed $\varepsilon > 0$,
